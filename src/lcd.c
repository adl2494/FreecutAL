...
int last_pen_pressure;
int last_stepper_speed;
...
{
	last_pen_pressure=timer_get_pen_pressure();
	last_stepper_speed=timer_get_stepper_speed();
}
void lcd_report_pressure(void)
{
	//keypadSet_Pressure_state( );
	int p=timer_get_pen_pressure();
	if (p != last_pen_pressure)
	{		
		lcd_clear();
		fprintf( &lcd, "Pressure: %d", p);
		last_pen_pressure=p;
	}
...
void lcd_report_speed(void)
{
	...
	if (p != last_stepper_speed)
	{
		lcd_clear();
		fprintf( &lcd, "Speed: %d", p);
		last_stepper_speed=p;
	}
}
